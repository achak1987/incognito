...
eTime<-c(401,388,72,480,584,130,662,704,156,703,779,178,940,1015,222,908,998,213,1002.5,1096,177,1097,1107,255,1479,1556,250,1577,1835,245,1581,1943,248)
...
plot(x=seq(1,length(bTime[,1])), y=bTime[,1], 
     type = "b", xaxt="n", xlab="Data Sizes", ylab="Bucketization (in seconds)", 
     ylim=c(0, max(bTime, na.rm=TRUE)), col="red", sub="(a)", pch=1)
axis(1, at=seq(1,length(bTime[,1])), labels=xLab)
lines(bTime[,2], type = "b", col="green", pch=2)
lines(bTime[,3], type = "b", col="blue", pch=3)

plot(x=seq(1,length(eTime[,1])), y=eTime[,1], 
     type = "b", xaxt="n", xlab="Data Sizes", ylab="Dichotomization (in seconds)", 
     ylim=c(0, max(eTime, na.rm=TRUE)), col="red", sub="(b)", pch=1)
axis(1, at=seq(1,length(eTime[,1])), labels=xLab)
lines(eTime[,2], type = "b", col="green", pch=2)
lines(eTime[,3], type = "b", col="blue", pch=3)

plot(x=seq(1,length(redTime[,1])), y=redTime[,1], 
     type = "b", xaxt="n", xlab="Data Sizes", ylab="Redistribution (in seconds)", 
     ylim=c(0, max(redTime, na.rm=TRUE)), col="red", sub="(c)", pch=1)
axis(1, at=seq(1,length(redTime[,1])), labels=xLab)
lines(redTime[,2], type = "b", col="green", pch=2)
lines(redTime[,3], type = "b", col="blue", pch=3)

plot(x=seq(1,length(recTime[,1])), y=recTime[,1], 
     type = "b", xaxt="n", xlab="Data Sizes", ylab="Recording (in seconds)", 
     ylim=c(0, max(recTime, na.rm=TRUE)), col="red", sub="(d)", pch=1)
axis(1, at=seq(1,length(recTime[,1])), labels=xLab)
lines(recTime[,2], type = "b", col="green", pch=2)
lines(recTime[,3], type = "b", col="blue", pch=3)

plot(x=seq(1,length(total[,1])), y=total[,1], 
     type = "b", xaxt="n", xlab="Data Sizes", ylab="Total Execution (in seconds)", 
     ylim=c(0, max(total, na.rm=TRUE)), col="red", sub="(e)", pch=1)
axis(1, at=seq(1,length(total[,1])), labels=xLab)
lines(total[,2], type = "b", col="green", pch=2)
lines(total[,3], type = "b", col="blue", pch=3)
...
reset()
legend("top", legend=c("incognito (beta=0.25)", "beta (beta=0.3)", "tclose (t=0.805)"), 
       pch=c(1,2,3),
       col=c("red", "green", "blue"),
       cex=0.75, ncol=3, bty="n")
